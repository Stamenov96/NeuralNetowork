...
class NeuralNetwork
{
  ...
  public double[] ComputeGradient(double[] yValues, double[] targets) {
		for(int i=0;i<yValues.length;i++){

			//System.out.println("targ : " + targets[i] +" MINUS "+yValues[i]);
			yValues[i]=(targets[i]-yValues[i])*(1-(yValues[i]))*(1+(yValues[i]));
			//System.out.println("RAVNO NA "+yValues[i]);
		}
	  return yValues;
	}
